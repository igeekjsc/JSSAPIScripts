...
case $authCheckChoice in
	[yY] | [Yy][Ee][Ss])
		...
		curl -k "$sourceJSS"JSSResource/categories --user "$sourceJSSuser:$sourceJSSpw" -H "Content-Type: text/xml" > "$localOutputDirectory"/authentication_check/raw.xml
		curlStatus=$?
		if (( $curlStatus == 0 ))
			then
	...
	then
		...
		do
			...
			echo -e "\nPosting $parsedXML_user ( $postInt_user out of $totalParsedResourceXML_user ) \n"
	 		curl -k -g "$destinationJSS"JSSResource/accounts/userid/99999 --user "$destinationJSSuser:$destinationJSSpw" -H "Content-Type: text/xml" -X POST -T "$xmlPost_user"
		done
		echo -e "\n\n----------\nPosting groups...\n"
		sleep 1
		...
		do
			...
			echo -e "\nPosting $parsedXML_group ( $postInt_group out of $totalParsedResourceXML_group ) \n"
	 		curl -k -g "$destinationJSS"JSSResource/accounts/groupid/99999 --user "$destinationJSSuser:$destinationJSSpw" -H "Content-Type: text/xml" -X POST -T "$xmlPost_group"
		done		
elif [ $jssResource = "computergroups" ]
	then 
		...
		do
			...
			echo -e "\nPosting $parsedXML_static ( $postInt_static out of $totalParsedResourceXML_staticGroups ) \n"
	 		curl -k -g "$destinationJSS"JSSResource/computergroups --user "$destinationJSSuser:$destinationJSSpw" -H "Content-Type: text/xml" -X POST -T "$xmlPost_static"
		done
		...
		do
			...
			echo -e "\nPosting $parsedXML_smart ( $postInt_smart out of $totalParsedResourceXML_smartGroups ) \n"
	 		curl -k -g "$destinationJSS"JSSResource/computergroups --user "$destinationJSSuser:$destinationJSSpw" -H "Content-Type: text/xml" -X POST -T "$xmlPost_smart"
		done
elif [ $jssResource = "advancedcomputersearches" ]
	then 
		...
		do
			...
			echo -e "\nPosting $parsedXML_smart ( $postInt_smart out of $totalParsedResourceXML_advancedComputerSearches ) \n"
	 		curl -k -g "$destinationJSS"JSSResource/advancedcomputersearches --user "$destinationJSSuser:$destinationJSSpw" -H "Content-Type: text/xml" -X POST -T "$xmlPost_smart"
		done
elif [ $jssResource = "advancedmobiledevicesearches" ]
	then 
		...
		do
			...
			echo -e "\nPosting $parsedXML_smart ( $postInt_smart out of $totalParsedResourceXML_advancedMobileDeviceSearches ) \n"
	 		curl -k -g "$destinationJSS"JSSResource/advancedmobiledevicesearches --user "$destinationJSSuser:$destinationJSSpw" -H "Content-Type: text/xml" -X POST -T "$xmlPost_smart"
		done
else
	...
	for parsedXML in $( ls "$localOutputDirectory"/"$jssResource"/parsed_xml )
		do
			...
			echo -e "\nPosting $parsedXML ( $postInt out of $totalParsedResourceXML ) \n"
	 		curl -k -g "$destinationJSS"JSSResource/$jssResource --user "$destinationJSSuser:$destinationJSSpw" -H "Content-Type: text/xml" -X POST -T "$xmlPost"
		done
fi
if [ $jssResource != "null" ]
	then
