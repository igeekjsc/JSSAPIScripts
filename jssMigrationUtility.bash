...
elif [ $jssResource = "policies" ] && [ $resourceNumber == 22 ]
	then
...
elif [ $jssResource = "policies" ] && [ $resourceNumber == 21 ]
	then
		echo -e "\n**********\n\nVery Important Info regarding Policies -- "
		echo -e "\n1. Policies that are not assigned to a category will NOT be migrated"
		echo "	Reason: want to avoid migrating one-off policies generated by Casper Remote "
		echo "2. The following items will not be migrated "
		echo "	a. Individual computers as a scope, exclusion, etc."
		echo "3. This will try and match Groups that policies were scoped to"
		echo "   Make sure if AD/LDAP groups have been used that you migrated and configured"
		echo "   them to WORK.  That means username and password has been set after option 1"
		echo "   and a sucessful test of the AD/LDAP servers has been carred out"
		echo "If you do not wish to match groups or have policies that fail to match but you still"
		echo "wish to migrate them please use the Policies migrate option inplace/after this."
		echo -e "\nThese items must be added manually via web app\n\n**********\n\n"
		read -p "Press RETURN key to acknowledge this message " returnChoice
		for resourceXML in $(ls "$localOutputDirectory"/"$jssResource"/fetched_xml)
			do
